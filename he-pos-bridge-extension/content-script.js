// HE POS Bridge — Content Script (ISOLATED world, v1.0)
...
//
// Runs on all HE POS tabs simultaneously:
//   POS 5  — Cash Counter
//   POS 6  — Captain
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'force-sync-attempt') {
    window.postMessage({ __he_bridge: true, type: 'force-sync', command_id: msg.command_id || null }, '*');
...
async function handleSnapshot(commandId, params) {
  ...
  await fetchViaSW('https://hamzaexpress.in/api/pos-health/snapshot', 'POST', {
...
function requestFromMain(type, payload) {
  ...
  return new Promise((resolve, reject) => {
    ...
    window.postMessage({ __he_bridge: true, type: 'main-command', main_type: type, command_id, payload }, '*');
  });
}
...
async function pollAndReport() {
  window.postMessage({ __he_bridge: true, type: 'request-state' }, '*');
...
async function getMachineIdFromSW() {
  ...
  return he_machine_id || 'he-unknown';
}
...
function start() {
  ...
  console.info('[HE-Bridge] content script ready');
}
