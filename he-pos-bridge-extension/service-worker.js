// HE POS Bridge — Service Worker (v1.0)
...
//
// Monitors up to 3 simultaneous HE POS tabs:
//   POS 5  — Cash Counter
//   POS 6  — Captain
//   POS 32 — Ground Floor Waiter Non-AC
//
...
const CFG = self.HE_BRIDGE_CONFIG;

const MACHINE_ID_KEY = 'he_machine_id';
const SECRET_KEY = 'he_bridge_secret';
const LOG_BUFFER_KEY = 'he_log_buffer';
const OFFLINE_BEACONS_KEY = 'he_offline_beacons';

const POS_URL_PATTERNS = [
  'https://test.hamzahotel.com/pos/ui*',
  'https://test.hamzahotel.com/odoo/pos*',
];
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  console.log('[HE-Bridge] installed v', chrome.runtime.getManifest().version);
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  } catch (e) { console.error('[HE-Bridge] alarm error', alarm.name, e); }
});
...
// Aggregates across all open POS tabs — each tab updates its slice keyed
// by configId. Unsynced count is the total across all POS.
let tabStates = {}; // configId → { unsynced, posOpen, lastSyncAttemptAt, lastSyncOk, ... }
let latestStatus = {
  online: true,
  posOpen: false,
  unsynced: 0,
...
async function handleStatusUpdate(payload, tab) {
  // Store per-tab state so we can aggregate
  const configId = payload.configId || tab?.id || 'unknown';
  tabStates[configId] = {
    ...payload,
    posOpen: true,
    tabId: tab?.id || null,
  ...
  // Aggregate: total unsynced, any error, latest sync attempt
  let totalUnsynced = 0;
  let anyError = null;
  ...
  for (const [, s] of Object.entries(tabStates)) {
    const age = (now - new Date(s.updatedAt).getTime()) / 1000;
    if (age > 180) continue; // stale tab state — ignore
    totalUnsynced += s.unsynced || 0;
    if (s.lastError) anyError = s.lastError;
    if (s.lastSyncAttemptAt && (!lastSyncAttemptAt || s.lastSyncAttemptAt > lastSyncAttemptAt)) {
      lastSyncAttemptAt = s.lastSyncAttemptAt;
    }
    if (s.lastSyncOk === false) allSyncOk = false;
  }
  ...
  latestStatus = {
    ...
    lastError: anyError,
    lastSyncAttemptAt,
    lastSyncOk: allSyncOk,
    updatedAt: new Date().toISOString(),
    activePosCount: Object.keys(tabStates).length,
  };
...
async function sendBeacon(reason) {
  ...
  const beacon = {
    ...
    reason,
    active_pos_count: status.activePosCount || null,
  };

  try {
    const res = await postToCloud(CFG.BEACON_PATH, beacon);
    if (!res.ok) console.warn('[HE-Bridge] beacon HTTP', res.status);
  } catch (e) {
    console.warn('[HE-Bridge] beacon offline, queueing:', e.message);
...
self.addEventListener('online', async () => {
  console.log('[HE-Bridge] connectivity restored');
  const { [OFFLINE_BEACONS_KEY]: queue = [] } = await chrome.storage.local.get([OFFLINE_BEACONS_KEY]);
...
async function flushLogs() {
  ...
  while (buf.length > 0) {
    ...
    try {
      ...
      if (!res.ok) { buf.unshift(...batch); break; }
    } catch (_) { buf.unshift(...batch); break; }
  }
  await chrome.storage.local.set({ [LOG_BUFFER_KEY]: buf });
}
...
async function dispatchCommand(cmd) {
  console.log('[HE-Bridge] dispatching command', cmd.id, cmd.type);
  switch (cmd.type) {
    case 'beacon-now':
    ...
    case 'force-sync': {
      const tabs = await chrome.tabs.query({ url: POS_URL_PATTERNS });
      if (tabs.length === 0) return submitCommandResult(cmd.id, null, 'no POS tab open');
      // If machine_id param specifies a tab, target just that one; else hit all
      const targetConfigId = cmd.params?.config_id || null;
      const replies = [];
    ...
    case 'reload-tab': {
      ...
      // Optionally target a specific POS by config_id
      const targetTabId = cmd.params?.tab_id || null;
      const toReload = targetTabId ? tabs.filter((t) => t.id === targetTabId) : tabs;
      for (const tab of toReload) await chrome.tabs.reload(tab.id);
      return submitCommandResult(cmd.id, { reloaded_tabs: toReload.map((t) => t.id) });
    }
    ...
    case 'set-log-level': {
      ...
      await chrome.storage.local.set({ he_log_level: level });
    ...
    case 'read-idb':
    case 'eval': {
      // If params.tab_id is provided, target that specific tab; otherwise use first POS tab
      const tabs = await chrome.tabs.query({ url: POS_URL_PATTERNS });
      if (tabs.length === 0) {
      ...
      // Prefer tab matching requested config_id, otherwise use first tab
      const preferredTabId = cmd.params?.tab_id || null;
      const targetTab = preferredTabId
        ? (tabs.find((t) => t.id === preferredTabId) || tabs[0])
        : tabs[0];
      try {
        const r = await chrome.tabs.sendMessage(targetTab.id, {
        ...
        return; // wait for async callback from content script
...
async function submitCommandResult(commandId, result, error) {
  try {
    ...
    console.log('[HE-Bridge] command', commandId, error ? 'failed:' + error : 'completed');
  } catch (e) { console.warn('[HE-Bridge] could not submit command result', e); }
}

// ── On reconnect, force all POS tabs to push pending orders ──────
